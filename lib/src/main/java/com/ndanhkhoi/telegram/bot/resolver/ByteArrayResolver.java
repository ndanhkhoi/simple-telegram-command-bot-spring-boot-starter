...
public class ByteArrayResolver extends TypeResolver<byte[]> {
    ...
    private ByteArrayResolver(Class<byte[]> type, Consumer4<Object, BotCommand, BotCommandArgs, TelegramLongPollingBot> resolver) {
        super(type, resolver);
    }
    ...
    public final static ByteArrayResolver INSTANCE = new ByteArrayResolver(byte[].class,
            (value, botCommand, botCommandArgs, telegramLongPollingBot) -> {
                ...
                SendMediaUtils.sendMedia(botCommandArgs.getUpdate().getMessage(), FileUtils.getInputFile((byte[]) value, "temp_" + FileUtils.getPosfixFileInstantByTime(ZoneId.systemDefault())), botCommandArgs.getUpdate().getMessage().getChatId(), sendFile, telegramLongPollingBot);
